...
func setup(points: Array, map_bounds: Dictionary, image_w: int, image_h: int, vehicle_color: Color, speed_kph_in: float, disappear_km_in: float, start_fraction: float = -1.0) -> void:
	...
	disappear_km = _rng.randf_range(50.0, 500.0)
	...
	# Visual marker (Polygon2D) so it's easy to recolor per vehicle type
	var poly = Polygon2D.new()
	var polygon = PackedVector2Array([Vector2(0, -10), Vector2(8, 10), Vector2(-8, 10)])
	poly.polygon = polygon
	poly.color = vehicle_color
	_pf.add_child(poly)
